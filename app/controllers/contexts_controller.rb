...
class ContextsController < ApplicationController
  ...
  def create_api
    @context = SHDM::Context.create(params[:context])
    @index  = SHDM::ContextIndex.find_all.select{|v| v.index_name.first == "#{@context.context_name}Idx"}.first
    render :json => {:status => :successful, :defaultIndex => @index.uri, :context => @context.uri}
  end
